...
def configure_logger(name, logger_level="INFO") -> logging.Logger:
    ...
    logger_level = logger_level if logger_level is not None else os.environ.get('LOGGER_LEVEL', "INFO")
...
class Logger:
    ...
    def __init__(self,
        ...
        self.base_dir = log_dir or os.environ.get('LOG_BASE_DIR', None) or os.path.dirname(
    ...
    def configure_logger(self):
        ...
        self.LOG_DIR = os.path.join(self.LOG_DIR, f"{self.log_id}_{datetime.today().strftime('%Y-%m-%d_%H%M%S')}.log")
        ...
        if not self.show_logger_id:
            self.log_format = (f"[%(asctime)s]: [pid-%(process)d]: [%(levelname)s]:[%(filename)s:%(lineno)d - "
                               f"%(funcName)2s()]: %(message)s")
            formatter = logging.Formatter(self.log_format)
        else:
            self.log_format = (f"[%(asctime)s]: [pid-%(process)d]:{self.logger_name} "
        ...
        self.logger_level = self.logger_level if self.logger_level is not None else os.environ.get('LOGGER_LEVEL',
                                                                                                   "INFO")
...
# dev logs
local_logger = Logger(log_id=os.getenv("LOG_ID", "mtpos"))
...
def log_attempt_number(retry_state):
    ...
    logger.error(f"Attempt #{retry_state.attempt_number}: Retrying after {retry_state.next_action.sleep} seconds.")
